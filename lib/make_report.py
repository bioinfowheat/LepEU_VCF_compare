...
add("<h4>Per-individual depth traces — bwa_hwe and bwa_nohwe (one panel each)</h4>")
add("""<p>The plots above average depth across the 14 samples. Here instead is the depth of <b>each
individual</b> (its own colour) along the chromosome, with <b>bwa_hwe (top)</b> and <b>bwa_nohwe (bottom)</b>
in separate, axis-aligned panels. Per-sample depth comes from <code>bcftools query -f
'%CHROM\\t%POS[\\t%DP]\\n'</code> binned to 50 kb windows.</p>""")
add(img("depth_per_individual.png","per-individual depth traces", maxw="100%"))
add("""<div class="key"><b>What this adds:</b> (1) the two panels are essentially identical — confirming, at
the per-sample level, that the HWE filter does not change coverage; (2) individuals vary in background
coverage (the band spans ~5–15×), useful for spotting a low-coverage sample that might drive missingness;
(3) the ≈7 Mb spike is not uniform — it is driven by a subset of individuals reaching ~40×, exactly the
per-sample signature of reads from a duplicated/collapsed region piling up in some samples, reinforcing that
the Fₛₜ peak there (§4b) is a mapping artefact rather than biology. (Generated by
<code>05_popgen/depth_per_individual.py</code>; change the VCF list at the top to compare any set.)</div>""")
